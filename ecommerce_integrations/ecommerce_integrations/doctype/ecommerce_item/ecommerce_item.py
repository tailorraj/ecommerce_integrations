    ...
    def check_unique_constraints(self) -> None:
        ...
        for filter in filters:
            if frappe.db.exists("Ecommerce Item", filter):
                frappe.throw(
                    _("Ecommerce Item already exists"), exc=frappe.DuplicateEntryError
                )
...
def is_meta_synced(integration: str, integration_item_code: str) -> bool:
    item_code = get_erpnext_item_code(integration, integration_item_code)
    if not item_code:
        return False
    fields = [
        "custom_reference",
